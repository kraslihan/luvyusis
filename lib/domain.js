...
export const MEAL_TYPES = ['Kahvaltı', 'Öğle Yemeği', 'Akşam Yemeği', 'Atıştırmalık', 'Diğer'];
export const EXERCISE_TYPES = ['Yürüyüş', 'Pilates', 'Spor Salonu', 'Koşu', 'Bisiklet', 'Yoga', 'Diğer'];

/* MET (metabolic equivalent) values used for the exercise calorie estimate.
   kcal = MET * weight(kg) * duration(hours). Rough, well-established averages. */
export const MET_BY_TYPE = {
  'Yürüyüş': 3.5, 'Pilates': 3.0, 'Spor Salonu': 5.0, 'Koşu': 9.8,
  'Bisiklet': 7.5, 'Yoga': 2.5, 'Diğer': 4.0,
};
export function estimateExerciseKcal(type, durationMin, weightKg) {
...
export function markCheckin(state, role) {
  ...
  day.checkin = true;
  if (!wasCheckedIn) {
    logActivity(state, '🌱', `${user.name} bugün check-in yaptı`);
    ...
    if (other.days[todayKey()] && other.days[todayKey()].checkin) {
      ...
      if (t > 0) logActivity(state, '🔥', `Birlikte seri ${t} güne ulaştı`);
    }
  }
}
...
export function applyAction(state, role, type, payload) {
  ...
  switch (type) {
    ...
    case 'ADD_MEAL': {
      ensureDay(user, key).meals.push({ id: uid(), type: payload.type, time: payload.time, desc: payload.desc, estimate: payload.estimate || null });
      markCheckin(state, role);
      break;
    }
    case 'EDIT_MEAL': {
      const d = ensureDay(user, key);
      const m = d.meals.find(m => m.id === payload.id);
      if (m) {
        if (payload.type != null) m.type = payload.type;
        if (payload.time != null) m.time = payload.time;
        if (payload.desc != null) m.desc = payload.desc;
        if ('estimate' in payload) m.estimate = payload.estimate || null;
      }
      break;
    }
    case 'DELETE_MEAL': {
    ...
    case 'ADD_EXERCISE': {
      const kcal = estimateExerciseKcal(payload.type, payload.duration, currentWeight(user));
      ensureDay(user, key).exercises.push({ id: uid(), type: payload.type, duration: payload.duration, note: payload.note || '', estimatedKcal: kcal });
      markCheckin(state, role);
      logActivity(state, '🏃', `${user.name} ${payload.duration} dakika ${payload.type} ekledi`);
      break;
    }
    case 'EDIT_EXERCISE': {
      const d = ensureDay(user, key);
      const ex = d.exercises.find(ex => ex.id === payload.id);
      if (ex) {
        if (payload.type != null) ex.type = payload.type;
        if (payload.duration != null) ex.duration = payload.duration;
        if (payload.note != null) ex.note = payload.note;
        ex.estimatedKcal = estimateExerciseKcal(ex.type, ex.duration, currentWeight(user));
      }
      break;
    }
    ...
    case 'EDIT_SHARED_NOTE': {
      const note = state.shared.notes.find(n => n.id === payload.id);
      if (!note) break;
      if (note.author !== role) throw Object.assign(new Error('Bu notu sadece yazan kişi düzenleyebilir.'), { statusCode: 403 });
    ...
    case 'DELETE_SHARED_NOTE': {
      const note = state.shared.notes.find(n => n.id === payload.id);
      if (!note) break;
      if (note.author !== role) throw Object.assign(new Error('Bu notu sadece yazan kişi silebilir.'), { statusCode: 403 });
      state.shared.notes = state.shared.notes.filter(n => n.id !== payload.id);
      break;
    }
    case 'REACT_NOTE': {
      const note = state.shared.notes.find(n => n.id === payload.id);
    ...
    case 'ADD_SONG': {
      ...
      logActivity(state, '🎵', `${user.name} bir şarkı önerdi`);
      break;
    }
    case 'EDIT_SONG': {
      const song = state.shared.songs.find(sg => sg.id === payload.id);
      if (!song) break;
      if (song.author !== role) throw Object.assign(new Error('Bu şarkıyı sadece ekleyen kişi düzenleyebilir.'), { statusCode: 403 });
      if (payload.song != null) song.song = payload.song;
      if (payload.artist != null) song.artist = payload.artist;
      if (payload.note != null) song.note = payload.note;
      break;
    }
    case 'DELETE_SONG': {
      const song = state.shared.songs.find(sg => sg.id === payload.id);
      if (!song) break;
      if (song.author !== role) throw Object.assign(new Error('Bu şarkıyı sadece ekleyen kişi silebilir.'), { statusCode: 403 });
      state.shared.songs = state.shared.songs.filter(sg => sg.id !== payload.id);
      break;
    }
    case 'ADD_WISH': {
    ...
    case 'RESET_MY_DATA': {
      state.users[role] = blankUser(user.name);
      state.onboarded[role] = false;
      break;
    }
    default:
